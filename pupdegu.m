...
file=strcat(PathName, FileName{16});
imagen=imread(file);
...
thresholdR= input('Enter right color threshold:  ');
thresholdL= input('Enter left color threshold:  ');
...
Diametrosvert(n,2)=n;
Area(n,1)=(VERTDIAMETER/2)*(DIAMETER/2)*pi; 
Area(n,2)=n; 
check=randi(30);
if check==25
   figure;
